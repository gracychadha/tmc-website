...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['action'])) {
		...
		if ($action === 'add') {
			...
			$status = isset($_POST['status']) ? 0 : 1; // Status is 1 for active, 0 for inactive1

			date_default_timezone_set('Asia/Kolkata');
			$date = date("Y-m-d");

			$stmt = $db->prepare('INSERT INTO faqs (question, answer, status, created_at) VALUES (?, ?, ?,?)');
			$stmt->bind_param('ssis', $question, $answer, $status, $date);
		...
		if ($_SERVER['REQUEST_METHOD'] === 'POST') {
			if (isset($_POST['action'])) {
				...
				if ($action === 'edit') {
					...
					$stmt = $db->prepare('UPDATE faqs SET question = ?, answer = ?, status = ?   WHERE faqs_id = ?');
					$stmt->bind_param('ssii', $question, $answer, $status, $id);
				...
				if ($action === 'delete') {
					...
					if (!empty($ids)) {
						...
						$stmt = $db->prepare("DELETE FROM faqs WHERE faqs_id IN ($placeholders)");
...
$stmt = $db->prepare("SELECT * FROM faqs ORDER BY faqs_id DESC");
$stmt->execute();
$result_faqs = $stmt->get_result();
...
<head>
	...
	<title>FAQs - Tee Mac Corporation</title>
			...
			<div class="content">
				...
				<div class="d-md-flex d-block align-items-center justify-content-between mb-3">
					<div class="my-auto mb-2">
						<h3 class="page-title mb-1">FAQs</h3>
						<nav>
				...
				<div class="card">
					<div class="card-header d-flex align-items-center justify-content-between flex-wrap pb-0">
						...
						<!-- <p class="small-text">Kindly add only 5 question answers </p> -->
					...
					<div class="card-body p-0 py-3">
						...
						<div class="custom-datatable-filter table-responsive">
							<table class="table datatable" data-status-col="3">
								...
								<tbody>
									<?php
									if ($result_faqs->num_rows > 0) {
										while ($rowfaqs = $result_faqs->fetch_assoc()) {
									?>
											<tr>
												<td>
													<div class="form-check form-check-md">
														<input class="form-check-input delete-checkbox" type="checkbox" value="<?php echo $rowfaqs['faqs_id']; ?>">
													</div>
												</td>
												<td>
												...
												<td>
													...
													echo implode(' ', array_slice(explode(' ', $rowfaqs['answer']), 0, 10)) . '...';

													?>
													<!-- <?php echo $rowfaqs['answer']; ?></td> -->
												<td>
													<?php if ($rowfaqs['status'] == 1) { ?>
												...
												<td>
													<div class="d-flex align-items-center">
														<a href="#" class="btn btn-outline-light bg-white btn-icon d-flex align-items-center justify-content-center rounded-circle edit-btn p-0 me-2" id="edit_faqs" data-bs-toggle="modal"
															data-id="<?php echo $rowfaqs['faqs_id']; ?>"
															data-question="<?php echo $rowfaqs['question']; ?>"
															data-answer="<?php echo $rowfaqs['answer']; ?>"
															data-status="<?php echo $rowfaqs['status']; ?>"
															data-bs-target="#edit-modal"><i class="ti ti-edit-circle text-primary"></i></a>
														<a href="#" class="btn btn-outline-light bg-white btn-icon d-flex align-items-center justify-content-center rounded-circle p-0 delete-btn me-3" data-id="<?php echo $rowfaqs['faqs_id']; ?>" data-bs-toggle="modal" data-bs-target="#delete-modal"><i class="ti ti-trash-x text-danger"></i></a>
													</div>
												</td>
											</tr>
									<?php }
		...
		<div class="modal fade" id="add_faqs">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<form id="faqsForm" action="admin-faqs.php" class="m-3" method="post">
						...
						<div class="mb-3">
							...
							<p class="small-text">Kindly add 3-4 lines answer only</p>
							<textarea class="form-control" placeholder="Enter Answer" name="answer" rows="4" required></textarea>
						</div>


						<div class="modal-footer">
							<button type="button" class="btn btn-light me-2" data-bs-dismiss="modal">Cancel</button>
							<button type="submit" name="add-submit" id="add-submit" class="btn btn-primary">Add FAQs</button>
						</div>
					</form>
		...
		<div class="modal fade" id="edit-modal" tabindex="-1" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<form action="admin-faqs.php" method="POST">
						<input type="hidden" name="action" value="edit">
						<div class="modal-body">
							<input type="hidden" name="id" id="edit-id">
							<div class="mb-3">
								<label for="edit-faqs-question" class="form-label">Question</label>
								<input type="text" class="form-control" placeholder="Enter Question" id="edit-question" name="question" required>
							</div>
							<div class="mb-3">
								<label for="edit-faqs-answer" class="form-label">Answer</label>
								<textarea class="form-control" placeholder="Enter Answer" id="edit-answer" name="answer" rows="4" required></textarea>
							</div>
			...
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					<form method="POST" action="admin-faqs.php">
						...
						<input type="hidden" name="ids" id="delete-faqs-ids" value=""> <!-- Hidden input for FAQ IDs -->
	...
	<script>
		$(document).ready(function() {
			$('.edit-btn').on('click', function() {
				...
				console.log("Clicked Edit for ID:", id); // Debugging output


				// Set the blog ID in the hidden input
				$('#edit-id').val(id);
				$('#edit-question').val(question);
				$('#edit-answer').val(answer);
				$('#edit-category-status').prop('checked', status == '1');
	...
	<script>
		$(document).ready(function() {
			$('.delete-btn').on('click', function() {
				...
				console.log("Clicked Delete for ID:", faqsId); // Debugging output

				// Set the FAQ ID in the hidden input
				$('#delete-faqs-ids').val(faqsId);
			...
			$('#delete-selected').click(function() {
				...
				if (selectedIds.length > 0) {
					...
					$('#delete-faqs-ids').val(selectedIds.join(','));

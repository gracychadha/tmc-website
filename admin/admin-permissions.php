...
$stmt = $db->prepare("SELECT * FROM menu ORDER BY id DESC");
$stmt->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	if (isset($_POST['action'])) {
		...
		// Add a new category
		if ($action === 'add') {
			$categoryName = $_POST['permission_name'];

			$stmt = $db->prepare('INSERT INTO menu (title) VALUES (?)');
		...
		if ($action === 'edit') {
			...
			$stmt = $db->prepare('UPDATE menu SET title = ? WHERE id = ?');
			$stmt->bind_param('si', $categoryName, $id);

			if ($stmt->execute()) {
				$_SESSION['message'] = 'Permisssion updated successfully!';
			} else {
				$_SESSION['message'] = 'Error: ' . $stmt->error;
			}
		}
...
<head>
	...
	<title>Permission - Tee Mac Corporation </title>
			...
			<div class="content">
				...
				<div class="d-md-flex d-block align-items-center justify-content-between mb-3">
					<div class="my-auto mb-2">
						<h3 class="page-title mb-1">Permission Management</h3>
						<nav>
						...
						<div class="custom-datatable-filter
 						table-responsive">
							...
							if ($result_category->num_rows > 0) {
								echo '<table class="table datatable" data-name-col="1" data-status-col="2">';
								...
								while ($data_category = $result_category->fetch_assoc()) {
									...
									echo '<a href="#" class="btn btn-outline-light bg-white btn-icon d-flex align-items-center justify-content-center rounded-circle p-0 me-3" data-bs-toggle="modal" data-bs-target="#delete-modal"  data-id="' . $data_category['id'] . '"
									data-name="' . htmlspecialchars($data_category['title']) . '"
		...
		<div class="modal fade" id="add_category">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<form id="categoryForm" action="admin-permissions.php" class="m-3" method="post">
						<input type="hidden" name="action" value="add">
						<div class="mb-3">
							<label class="form-label">Permission Name</label>
							<input type="text" class="form-control" placeholder="Enter Permission Name" name="permission_name" required>
						</div>
					...
					<div id="successMessage" class="tmc-hidden alert alert-success mt-3">Permission added successfully!</div>
		...
		<div class="modal fade" id="edit_category" tabindex="-1" aria-hidden="true">
			<div class="modal-dialog modal-dialog-centered">
				<div class="modal-content">
					...
					<form action="admin-permissions.php" method="POST">
						<input type="hidden" name="action" value="edit">
						<div class="modal-body">
							<input type="hidden" name="id" id="edit-category-id">
							<div class="mb-3">
								<label for="edit-category-name" class="form-label">Permission Name</label>
								<input type="text" class="form-control" id="edit-category-name" name="permission_name" placeholder="Enter Category">
							</div>
				...
				<div class="modal-content">
					<form action="admin-permissions.php" method="POST">
						<div class="modal-body text-center">
							...
							<!-- Description -->
							<p>You want to delete this Permission, this can't be undone once you delete.</p>

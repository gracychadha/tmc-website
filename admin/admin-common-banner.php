...
$hero = null;
$stmt = $db->prepare("SELECT * FROM common_banner LIMIT 1");
$stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['hero_submit'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if ($check !== false) {
            if (move_uploaded_file($_FILES["image"]["tmp_name"], $targetFile)) {
                ...
                header("Location: admin-common-banner.php");
                exit();
            }
        } else {
    ...
    if ($hero) {
        ...
        $stmt = $db->prepare('UPDATE common_banner SET title = ?, image = ?, status = ? WHERE idcommon_banner = ?');
        $stmt->bind_param('sssi', $mainTitle, $image, $status, $hero['idcommon_banner']);
    } else {
        // Insert new record
        $stmt = $db->prepare('INSERT INTO common_banner (title, image, status) VALUES (?, ?, ?)');
        $stmt->bind_param('ssi',$mainTitle , $image, $status);
    }
    
    if ($stmt->execute()) {
        $_SESSION['message'] = "Common banner updated successfully.";
    } else {
...
<head>
    ...
    <title>Common Banner - Tee Mac Corporation</title>
            ...
            <div class="content">
                <div class="d-md-flex d-block align-items-center justify-content-between border-bottom pb-3">
                    <div class="my-auto mb-2">
                        <h3 class="page-title mb-1">Common Banner</h3>
                        <nav>
                            <ol class="breadcrumb mb-0">
                                <li class="breadcrumb-item"><a href="dashboard.php">Dashboard</a></li>
                                <li class="breadcrumb-item active" aria-current="page">Common Banner</li>
                            </ol>
                        </nav>
                    </div>
                        ...
                        <div class="container mt-5">
                            <form action="" method="POST" enctype="multipart/form-data">
                                <div class="about-container container p-4 bg-light rounded shadow-sm mt-5">
                                    <h5 class="mb-3">Common Banner Settings</h5>
                                    <div class="row col-12">
                                        <div class="col-md-6 mb-3">
                                            <label for="title" class="form-label"> Title *</label>
                                            <input type="text" class="form-control" id="main_title" name="title" placeholder="e.g. We Create Events That People Remember" value="<?php echo htmlspecialchars($hero['title'] ?? ''); ?>" required>
                                        </div>

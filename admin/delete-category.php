...
if (isset($_GET['id'])) {
    ...
    if ($delete_result) {
        ...
        header("location: manage-category.php?status=" . base64_encode(1)); // Redirect with success status
        exit(); // Terminate script execution after redirection
    } else {
        // Error occurred while deleting category
        header("location: manage-category.php?status=" . base64_encode(-1)); // Redirect with error status
        exit(); // Terminate script execution after redirection
    }
}
...
header("location: manage-category.php?status=" . base64_encode(-1));
exit(); // Terminate script execution after redirection
?>

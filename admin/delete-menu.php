...
if (isset($_GET['id'])) {
    ...
    if ($delete_result) {
        ...
        header("location: manage-menu.php?status=" . base64_encode(1)); // Redirect with success status
        exit(); // Terminate script execution after redirection
    } else {
        // Error occurred while deleting category
        header("location: manage-menu.php?status=" . base64_encode(-1)); // Redirect with error status
        exit(); // Terminate script execution after redirection
    }
}
...
if (isset($_POST['menu_ids'])) {
    ...
    $success_count = 0;
    foreach ($menu_ids as $encoded_id) {
        $menu_id = base64_decode($encoded_id);
        $delete_query = "DELETE FROM menu WHERE menu_id = '$menu_id'";
        $delete_result = mysqli_query($db, $delete_query);
...
header("location: manage-menu.php?status=" . base64_encode(-1));
exit(); // Terminate script execution after redirection
?>

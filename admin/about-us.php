...
// Fetch existing data
$about = null;
$stmt = $db->prepare("SELECT * FROM about_about LIMIT 1");
$stmt->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['about_submit'])) {
    ...
    if ($about) {
        // Update
        $stmt = $db->prepare('UPDATE about_about SET title = ?, sub_title = ?, content = ?, benefits = ?, image = ?, image2 = ? WHERE id = ?');
        $stmt->bind_param('ssssssi', $title, $subTitle, $content, $benefits, $image, $image2, $about['id']);
    } else {
        // Insert
        $stmt = $db->prepare('INSERT INTO about_about (title, sub_title, content, benefits, image, image2) VALUES (?, ?, ?, ?, ?, ?)');

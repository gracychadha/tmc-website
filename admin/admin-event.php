...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Check if the form is being submitted for adding a new event
    if (isset($_POST['add-form']) && $_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $slug = generateSlug($eventTitle); // Generate slug from title
        ...
        if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] == 0) {
            $targetDir = "event/"; // Define the directory for uploading the image
        ...
        $stmt = $db->prepare("INSERT INTO event (title, description, image, date, image_link, program_link, status, slug) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssssss", $eventTitle, $description, $featuredImage, $date, $imageLink, $programLink, $status, $slug);
    ...
    if (isset($_POST['edit-form']) && $_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $stmt = $db->prepare("UPDATE event SET title = ?, slug = ?, status = ?, description = ?, image = ?, date = ?, image_link = ?, program_link = ? WHERE idevent = ?");
        $stmt->bind_param("ssssssssi", $title, $slug, $status, $content, $image_path, $date, $imageLink, $programLink, $idevent);
                    ...
                    <div class="custom-datatable-filter table-responsive">
                        <table class="table datatable">
                            ...
                            <tbody>
                                ...
                                ?>
                                        <tr>
                                            <td><div class="form-check form-check-md"><input class="form-check-input delete-checkbox" type="checkbox" value="<?php echo $rowevents['idevent']; ?>"></div></td>
                                            <td><div class="d-flex align-items-center"><div class="ms-2"><p class="text-dark mb-0"><?php echo $rowevents['title']; ?></p></div></div></td>
                                            <td>

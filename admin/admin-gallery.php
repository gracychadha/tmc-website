...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add-form'])) {
        ...
        if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] == 0) {
            $targetDir = "gallery/"; // Directory for slider images
        ...
        $stmt = $db->prepare("INSERT INTO gallery (image, status, created_at) VALUES (?, ?, ?)");
    ...
    if (isset($_POST['edit-slider'])) {
        ...
        $stmt = $db->prepare("UPDATE gallery SET image = ?, status = ? WHERE idgallery = ?");
        $stmt->bind_param("sii", $image_path, $status, $idgallery);
    ...
    if (isset($_POST['delete-form'])) {
        if (!empty($_POST['ids'])) {
            ...
            $stmt = $db->prepare("DELETE FROM gallery WHERE idgallery IN ($placeholders)");
            ...
            if ($stmt->execute()) {
                $_SESSION['message'] = ($stmt->affected_rows > 0)
                    ? "Gallery image(s) deleted successfully!"
                    : "No Gallery image found with those IDs.";
            } else {
                $_SESSION['message'] = "Error deleting Gallery image(s): " . $stmt->error;
            }

            $stmt->close();
        } else {
            $_SESSION['message'] = "Invalid Gallery IDs.";
        }
...
$slider_query = "SELECT * FROM gallery ORDER BY idgallery DESC";
...
<head>
    ...
    <title>Gallery - Tee Mac Corporation</title>
    ...
    <div class="main-wrapper">
            ...
            <div class="content">
                ...
                <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
                    <div class="my-auto mb-2">
                        <h3 class="page-title mb-1">Gallery</h3>
                    ...
                    <div class="d-flex my-xl-auto right-content align-items-center flex-wrap">
                        ...
                        <div class="mb-2">
                            <a href="#" class="btn btn-primary d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#add_slider"><i class="ti ti-square-rounded-plus me-2"></i>Add Gallery</a>
                        </div>
                    </div>
                </div>
                <!-- /Page Header -->
                                                ...
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        <a href="#" class="btn btn-outline-light bg-white btn-icon edit-btn d-flex align-items-center justify-content-center rounded-circle p-0 me-2"
                                                            data-id="<?php echo $rowslider['idgallery']; ?>"
                                                        ...
                                                        <a href="#" class="btn btn-outline-light bg-white btn-icon d-flex align-items-center justify-content-center delete-btn rounded-circle p-0 me-3" data-id="<?php echo $rowslider['idgallery']; ?>" data-bs-toggle="modal" data-bs-target="#delete-modal"><i class="ti ti-trash-x text-danger"></i></a>
                                                    </div>
                                                </td>
                                            </tr>
                                    <?php }
            ...
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title">New Gallery Image</h4>
                    ...
                    <form action="admin-gallery.php" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" name="add-form" class="btn btn-primary">Add </button>
                        </div>
                    </form>
                </div>
            </div>
        ...
        <div class="modal fade" id="edit_slider" tabindex="-1" aria-labelledby="edit_sliderLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="edit_sliderLabel">Edit Gallery Image</h4>
                        <button type="button" class="btn-close custom-btn-close" data-bs-dismiss="modal" aria-label="Close">
                            <i class="ti ti-x"></i>
                        </button>
                    </div>
                    <form id="sliderForm" action="admin-gallery.php" method="POST" enctype="multipart/form-data">
                        <div class="modal-body">
                            <div class="row">
                                <!-- Hidden input for slider ID -->
                                <input type="hidden" name="idgallery" id="edit-slider-id">
                                <!-- Image upload -->
                                <div class="col-md-6 mb-3">
                                    <label class="form-label">Image</label>
                                    <div class="d-flex align-items-center upload-pic flex-wrap row-gap-3">

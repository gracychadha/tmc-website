...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add-user'])) {
        ...
        if (empty($errors)) {
            ...
            $permissionJson = json_encode($permission); // Convert permissions to JSON

            // Insert the data into the database
            $stmt = $db->prepare("INSERT INTO admin (username, email, phone, password, date, activation, status, permission, expire_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param('sssssssss', $username, $email, $mobile, $hash, $current_datetime, $activationToken, $status, $permissionJson, date('Y-m-d', $expire_time));
            if ($stmt->execute()) {
    ...
    if (isset($_POST['edit-user']) && isset($_POST['edit_id'])) {
        ...
        $sql = "UPDATE admin SET username = ?, email = ?, phone = ?, status = ?, permission = ? WHERE admin_id = ?";
        $stmt = $db->prepare($sql);
        $stmt->bind_param('sssssi', $username, $email, $mobile, $status, $permissionJson, $id);
        if ($stmt->execute()) {
            $_SESSION['msg'] = "User record updated successfully!";
            $_SESSION['msg_type'] = 'success';
            header("location:user-permission.php");
        } else {
...
$permission = "SELECT * FROM menu";
$result_permission = $db->query($permission);
?>
            ...
            <div class="content">
                <div class="d-md-flex d-block align-items-center justify-content-between mb-3">
                    <div class="my-auto mb-2">
                        <h3 class="page-title mb-1">User Permission</h3>
                        <nav>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="permission[]" id="edit-permission" class="form-control select2" multiple>
                                    ...
                                    while ($row_permission = mysqli_fetch_assoc($result_permission)) {
                                        echo '<option value="' . $row_permission['id'] . '">' . htmlspecialchars($row_permission['title']) . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
